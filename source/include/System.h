...
namespace tx {
    ...
    public:
         ...
         */
        virtual bool isInterested(const Context&, const EntityID&, const ComponentID&) const { return false; };
    ...
    public:
        ...
        AspectSpecificSystem(std::array<ComponentID, sizeof...(C)>&& cIds, bool valid = false)
            : System< AspectSpecificSystem<Aspect_<C...>>>(valid)
            , aspect(std::move(cIds))
        {	}

        virtual ~AspectSpecificSystem() {
        }

        virtual bool isInterested(const Context&, const EntityID& eId, const ComponentID& cId) const override;
