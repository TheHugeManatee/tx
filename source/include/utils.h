...
template<typename T, typename NameSeed,
         typename Enable = typename std::conditional<!std::is_fundamental<T>::value, std::true_type,
                                                     std::false_type>::type>
...
{
public:
    explicit ExecAtScopeExit(Func fn_) : fn(fn_){};
...
public:
    ...
    ResetAfterScope(VariableType& tgt, const ValueType& value) : target(tgt), valueBefore(tgt)
    {
        target = value;
    }
    ~ResetAfterScope() { target = valueBefore; }
    // Todo: Think about moving operations, could be handled with either a flag or storing target as
    // a pointer instead

    template<typename ValueType2>
    ResetAfterScope& operator=(const ValueType2& rhs)
    {
        target = rhs;
    }
    template<typename ValueType2>
    ResetAfterScope& operator=(ValueType2&& rhs)
    {
        target = rhs;
    }
...
#define SET_TEMPORARILY(variable, value)                                                           \
    auto temp_##variable = tx::setTemporarily((variable), (value))
...
{
    std::cout << "Assertion \"" << condition << "\" in (" << fileName << ", " << functionName
              << ", Ln " << lineNumber << "): ";
...
#define txAssert(condition, message)                                                               \
    {                                                                                              \
        ...
        stream__ << message;                                                                       \
        if (!(bool(condition)))                                                                    \
            tx::txAssertion(__FILE__, __FUNCTION__, __LINE__, #condition, (stream__.str()));       \
    }
#else
#define txAssert(condition, message)
#endif
