...
namespace tx {
    ...
    public:
        ...
        bool checkAspect(const Entity& entity) const noexcept {
            ...
            for (auto flag : componentFlags) if (!flag) return false;
            return true;
        }
        ...
        bool isPartOf(const ComponentID& cId) const {
            // TODO review this, might be unsafe cause of this funny idx++ which is unsequenced. Replace with "swallow" pattern
            size_t idx = 0;
            bool componentFlags[] = { (cId == ids_[idx++] && std::is_same<ComponentTypes, ComponentType>::value)...};
            for (auto flag : componentFlags) if (flag) return true;
            return false;
        }

        bool isIDPartOf(const ComponentID& cId) const {
            for (auto id : ids_) if (id == cId) return true;
            return false;
        }
...
namespace tx {
    ...
    //template<class... ComponentTypes>
    //AspectInstance<ComponentTypes...> make_aspect_instance(std::pair<ComponentID, std::add_pointer<ComponentTypes>::type>&&... args)
    //{
